...
def import_model_data(filename: str) -> (pd.DataFrame):
    ...
    vectorized_data = retrieve_glove_embeddings(model_data)
...
def model_results(clfs: list, X: pd.DataFrame, y: pd.Series,
    ...
    for clf in clfs:
        ...
        for test_size in test_sizes:
            base_model(clf, X, y, test_size, 42, use_smote=True)
            base_model(clf, X, y, test_size, 42, use_smote=False)
            cv = sklearn.model_selection.StratifiedKFold(n_splits=5, 
                                      random_state=42, shuffle=True)
            
        clf_scores, clf_scores_mean = run_cross_val_model(clf, X, y, cv)
        plot_my_cross_val_roc_curve(clf, X, y, cv, smote)
